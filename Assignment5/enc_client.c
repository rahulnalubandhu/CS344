...
Date: 12/04/2022
Copied from /OR/ Adapted from /OR/ Based on: Source URL: Initial skeleton was taken from https://replit.com/@cs344/83clientc?lite=true#client.c
later few fuctions are refrenced from class notes 4.2 and 4.3 lecture slides
* Client code
...
int main(int argc, char *argv[])
{
	...
	portNumber = atoi(argv[3]); // Get the port number and convert to an integer from a string
	...
	char* msg = "enc_client_connect_req";
	...
	//return message from server for connection request
	...
	if(strcmp(buffer, "no") == 0) 
		error("enc_client : Bad client : Connection refused from server!");		
	...
	// //acknowledge msg sent  //for debuging
	...
	{	
		...
		strcat(ciphertext, buffer); // concatinate the recieved characters to cipher text
	}
	...
	//adding new line charc to end of cipher
	strcat(ciphertext, "\n");
